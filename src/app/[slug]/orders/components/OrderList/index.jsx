      ...
      {orders.map((order) => (
        <Card key={order.id}>
          <CardContent className="space-y-4 p-5">
            <div
              className={`w-fit rounded-full px-2 py-1 text-xs font-semibold text-white ${[OrderStatus.PAYMENT_CONFIRMED, OrderStatus.FINISHED].includes(order.status) ? "bg-green-500 text-white" : "bg-gray-200 text-gray-500"} `}
            >
